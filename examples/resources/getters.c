...
int main() {
    // Removed DtwNamespace dtw = newDtwNamespace(); as we're using pure C functions directly

    DtwResource *values = new_DtwResource("tests/target");

    printf("elements:---------------------------------\n");
    DtwStringArray *sub_elements = DtwResource_list_names(values);
    DtwStringArray_sort(sub_elements);  // This is already a pure C function
    ...
    if (!DtwResource_error(values)) {
        ...
        printf("blob size: %ld\n", blob_r->value_size);  // Note: Assuming this is correct based on context
