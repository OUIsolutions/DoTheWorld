...
darwin.add_recipe({
    name="declaration",
    description="Builds the DoTheWorld.h file with the declaration of the project",
    callback=build_declaration,
    requires={"dir_project"},
    outs={"release/DoTheWorld.h"}
})
...
function build_definition()
    ...
    darwin.dtw.write_file("release/DoTheWorld.c", onefile)

end 
darwin.add_recipe({
    name="onefile",
    description="Builds the DoTheWorld.c file with all the dependencies inside",
    callback=build_definition,
    requires={"dir_project"},
    outs={"release/DoTheWorld.c"}
})
